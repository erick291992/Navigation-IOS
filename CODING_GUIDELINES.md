...
- ❌ Parallel data fetches that "happen to overlap" for what visually appears to be the same value across multiple UI elements. Concrete example: the picker once had a library-wide `recentAssets` query feeding the previewer's first paint + the gallery shortcut, AND an album-scoped grid fetch. On the default Recents view they overlapped — on any other default album they diverged silently (the previewer would show a library-wide recent that wasn't even in the active album). When multiple UI elements display "the same conceptual value," derive them from a single source of truth (here: the album's first asset). Pick the more specific data source that serves all consumers; don't ship two fetches that "look identical on the default." See `MEDIA_PICKER_GUIDELINES.md` "What feeds each UI element."
